...
class Stationery:
    ...
    def _is_valid_phone_number(phone_number: str):
        result = (isinstance(phone_number, str) and
                  phone_number.startswith("+7") and
                  len(phone_number) == 12)
        return result
...
class Pencil(Stationery):
    def __new__(cls, *args, **kwargs):
        ...
        if not (cls._is_valid_initial_parameters(kwargs)):
            logging.error(f"""Неверные данные для инициализации!
                          Объект с данными {kwargs} не был создан из-за ошибки!""")
            return None
    ...
    def __str__(self):
        return f"""Объект pencil. Параметры --> lead_density: {self.lead_density},
...
class Pen(Stationery):
    ...
    def _is_valid_initial_parameters(cls, parameters: dict) -> bool:
        if not (super()._is_valid_str_param_of_child_enum(
            parameters.get("pen_type"),
            cls.available_pen_types
        )):
            logging.error(f"""Параметр типа ручки (pen_type) задан неверно.
                          Допустимые значения: {cls.available_pen_types}.
                          Переданное значение: {parameters.get("pen_type")}""")
            return False
        
        if not super()._is_positive_float_param_of_child(
            parameters.get("rod_diameter")
        ):
    ...
    def __str__(self):
        return f"""Объект Pen. Параметры --> rod_diameter: {self.rod_diameter},
        pen_type: {self.pen_type}, price: {self.price},
        phone_number: {self.phone_number}\n"""
...
class Paper(Stationery):
    ...
    def _is_valid_initial_parameters(cls, parameters: dict) -> bool:
        ...
        if not (super()._is_positive_int_param_of_child(parameters.get("height"))):
            logging.error("""Параметр высоты бумаги (height) указан неверно.
                          Высота должна быть целым положительным числом""")
            return False
    ...
    def __str__(self):
        return f"""Объект Paper. Параметры --> weight: {self.weight},
        height: {self.height},
        width: {self.width},
        price: {self.price},
        phone_number: {self.phone_number}\n"""
